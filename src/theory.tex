...
Условие $\delta = \omega_0$ определяет критический режим колебаний, 

\subsection{Вынужденные колебания в электрическом контуре}
\subsubsection*{Метод векторных диаграмм}
\begin{gather}
	\label{eq:th:19} U_L = I_0 \omega L = \frac{\omega L E_0}{\sqrt{R^2 + \left(\omega L - \frac{1}{\omega C}\right)^2}}\\
	\label{eq:th:20} U_C = \frac{I_0}{\omega C} = \frac{E_0}{\omega C \sqrt{R^2 + \left(\omega L - \frac{1}{\omega C}\right)^2}}\\
	\label{eq:th:21} U_R = I_0 R = \frac{E_0 R}{\sqrt{R^2 + \left(\omega L - \frac{1}{\omega C}\right)^2}}
\end{gather}

\subsubsection{Анализ / Вывод}
